...
import com.tribalfs.stargazers.data.datastore.appPreferences
import com.tribalfs.stargazers.data.room.StargazersDB
...
//Single source of truth for stargazers data
    ...
    private suspend fun fetchStargazers(): List<Stargazer>? = withContext(Dispatchers.IO){
        ...
        try {
            with(RetrofitClient.instance) {
                repoList.map {
                    async { getStargazers("tribalfs", it) }
                }.awaitAll()
                    .flatten()
                    .distinct()
                    .map {
